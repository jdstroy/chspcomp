"cHspComp ver.0.3.2" readme　（2005/9/10作成）
...
　タなどからHSPスクリプトの実行／コンパイル／スクリプトからの実行ファイル作成を
　行うためのソフトです。ソースのみの形で公開しています。
...
　・変更したものや部分的に使用したものは、あなたのものになります。公開する場合は、
　　あなたの名前の下で行って下さい。
...
　・cHspCompは目的の機能を実現するためにhspcmp.dllを利用していますが、HSP 3.0の
　　hspcmp.dllには、文法間違いを含むスクリプトをコンパイルしようとすると、強制終
　　了してしまうというバグがあります。これを回避するためにHSPの"common"フォルダ
　　にある"hspcmp.as"の内容を以下のように修正して下さい。

　　（修正前）
　　> #func hsc_comp hsc_comp 0

　　　　　　　↓
　　（修正後）
　　> #func hsc_comp "_hsc_comp@16" int,int,int,int

　　次のバージョンのHSPではこの修正を反映済みの"hspcmp.as"が同梱される予定です。
...
　・ソースから作成したchspcomp.exeを、HSP3がインストールされているフォルダ（=hsp3
　　.exeがあるフォルダ）にコピー。

　　（cHspEditと組み合わせて使う用途のみであればこれで完了です）
